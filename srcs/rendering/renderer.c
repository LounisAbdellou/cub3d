...
/*   Updated: 2024/10/17 01:35:32 by solid_42         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_wall(t_env *env, float ray_angle, int ray_i, float delta)
{
	double	origin;
	double	end;
	double	height;

	delta *= cos(nor_angle(ray_angle - env->player.angle));
	height = (TILE_SIZE / delta) * ((env->screen_w / 2) / tan(FOV_RD / 2));
...
void	render_walls(t_env *env, t_point pos)
{
	...
	{
		inter = get_best_inter(env, pos, angle);
		draw_wall(env, angle, i, inter);
